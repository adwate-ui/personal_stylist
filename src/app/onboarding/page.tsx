...
import { Upload, ChevronRight, Check, ArrowLeft, ArrowRight, Ruler, Palette, Briefcase, Sparkles, User, Shirt, Loader2, Heart } from "lucide-react";
...
const getProductImagePlaceholder = (item: string) => {
    const lower = item.toLowerCase();
    if (lower.includes('shirt')) return '👕';
    if (lower.includes('pant') || lower.includes('jeans') || lower.includes('trousers')) return '👖';
    if (lower.includes('shoe') || lower.includes('boot') || lower.includes('sneaker')) return '👟';
    if (lower.includes('jacket') || lower.includes('coat') || lower.includes('blazer')) return '🧥';
    if (lower.includes('dress')) return '👗';
    if (lower.includes('bag') || lower.includes('tote')) return '👜';
    if (lower.includes('glasse')) return '🕶️';
                        ...
                        {styleDNA.generated_by_model && (
                            <div className="mt-4 inline-flex items-center gap-2 px-4 py-2 bg-white/5 border border-white/10 rounded-lg text-sm">
                                ...
                                <span className="text-primary font-medium">
                                    {!['gemini-3-pro-preview', 'gemini-2.5-pro', 'gemini-3-flash-preview', 'gemini-2.5-flash'].includes(styleDNA.generated_by_model) ? styleDNA.generated_by_model : styleDNA.generated_by_model.replace(/-/g, ' ')}
                                </span>
                                <a
                            ...
                            <div className="space-y-6">
                                {Object.entries(styleDNA.must_have_staples).map(([category, types]) => (
                                    <div key={category}>
                                        ...
                                        {typeof types === 'object' && types !== null ? Object.entries(types).map(([type, items]) => (
                                            <div key={type} className="mb-6 last:mb-0">
                                                <h5 className="text-xs text-gray-500 uppercase tracking-wider mb-2">{type.replace(/_/g, ' ')}</h5>
                                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                                    {Array.isArray(items) && items.map((item: { item: string; brand: string; why: string; product_url: string; color?: string }, i: number) => (
                                                        <div key={i} className="bg-white/5 rounded-xl border border-white/5 hover:border-primary/30 transition-all group overflow-hidden">
                                                            <a href={item.product_url || '#'} target="_blank" rel="noopener noreferrer" className="block relative overflow-hidden bg-white/10 aspect-square">
                                                                <div className="absolute inset-0 flex items-center justify-center text-5xl">
                                                                    {getProductImagePlaceholder(item.item)}
                                                                </div>
                                                            </a>
                                                            <div className="p-3">
