...
export default function AddItemPage() {
    ...
    const handleLinkSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            try {
                // Guard: Don't send Google Search URLs to AuthentiQC
            ...
            try {
                if (data.imageBase64) {
                    ...
                    try {
                        ...
                        console.log('[Add Item] Successfully fetched imageUrl direct');
                    } catch (directError) {
                        console.warn('[Add Item] Direct fetch failed (likely CORS), trying proxy...', directError);
                        try {
                            // Use wsrv.nl as a reliable, fast image proxy
                            const proxyUrl = `https://wsrv.nl/?url=${encodeURIComponent(imageUrlToFetch)}`;
                            const proxyResponse = await fetch(proxyUrl);
                            if (!proxyResponse.ok) throw new Error(`Proxy Status ${proxyResponse.status}`);
                            imgBlob = await proxyResponse.blob();
                            console.log('[Add Item] Successfully fetched imageUrl via proxy');
                        } catch (proxyError) {
                            throw new Error("Failed to fetch image directly and via proxy");
                        }
                    }
                } else {
